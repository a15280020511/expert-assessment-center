...
IGNORED = {".git", "__pycache__", ".pytest_cache", ".ruff_cache", "audit-artifacts", "artifacts", "validation-artifacts", "ticket-artifacts", "runtime-state", "performance-state", "tmp-test-artifacts"}
...
def audit_python(rel: str, text: str) -> tuple[list[Finding], set[str], dict[str, int]]:
    ...
    try:
        ...
        return [Finding("critical", "PY-SYNTAX", rel, int(exc.lineno or 1), str(exc))], imports, metrics
    for node in ast.walk(tree):
        if isinstance(node, (ast.FunctionDef, ast.AsyncFunctionDef)):
            ...
            if score > 20: findings.append(Finding("medium", "PY-COMPLEXITY", rel, node.lineno, f"function {node.name!r} complexity={score}"))
            if length > 180: findings.append(Finding("medium", "PY-FUNCTION-SIZE", rel, node.lineno, f"function {node.name!r} spans {length} lines"))
        elif isinstance(node, ast.ClassDef): metrics["classes"] += 1
        elif isinstance(node, ast.Import): imports.update(alias.name.split(".")[0] for alias in node.names)
        elif isinstance(node, ast.ImportFrom) and node.module: imports.add(node.module.split(".")[0])
        elif isinstance(node, ast.ExceptHandler):
            if node.type is None: findings.append(Finding("high", "PY-BARE-EXCEPT", rel, node.lineno, "bare except hides termination and programming errors"))
            elif isinstance(node.type, ast.Name) and node.type.id == "BaseException": findings.append(Finding("high", "PY-BASE-EXCEPTION", rel, node.lineno, "BaseException handler catches process termination"))
        elif isinstance(node, ast.Call):
            func = node.func
            if isinstance(func, ast.Name) and func.id in {"eval", "exec"}: findings.append(Finding("critical", "PY-DYNAMIC-CODE", rel, node.lineno, f"use of {func.id}()"))
            if isinstance(func, ast.Attribute) and isinstance(func.value, ast.Name):
                owner, name = func.value.id, func.attr
                if owner == "os" and name == "system": findings.append(Finding("critical", "PY-OS-SYSTEM", rel, node.lineno, "os.system executes a shell"))
                if owner == "subprocess" and any(k.arg == "shell" and isinstance(k.value, ast.Constant) and k.value.value is True for k in node.keywords): findings.append(Finding("critical", "PY-SHELL-TRUE", rel, node.lineno, "subprocess call uses shell=True"))
                if owner in {"pickle", "dill"} and name in {"load", "loads"}: findings.append(Finding("high", "PY-UNSAFE-DESERIALIZE", rel, node.lineno, f"{owner}.{name} can execute untrusted input"))
            ...
            if isinstance(value, ast.Constant) and isinstance(value.value, str):
                ...
                if names and any(SECRET_NAME.search(name) for name in names) and len(value.value) >= 12 and not PLACEHOLDER.search(value.value):
...
def audit(root: Path) -> dict[str, Any]:
    ...
    for path in sorted(root.rglob("*")):
        ...
        for index, line in enumerate(text.splitlines(), 1):
            ...
            if kind == "yaml" and re.match(r"\s*uses:\s*", line):
                ...
                if not action.startswith(("./", "docker://")) and not PINNED_ACTION.match(action): findings.append(Finding("high", "GHA-UNPINNED-ACTION", rel, index, f"action is not pinned to a full commit SHA: {action}"))
        if kind == "python":
            py_findings, imports, py_metrics = audit_python(rel, text)
            findings.extend(py_findings); imports_by_file[rel] = imports; metrics[rel] = py_metrics
        elif kind == "json":
            try: json.loads(text)
            except json.JSONDecodeError as exc: findings.append(Finding("critical", "JSON-PARSE", rel, exc.lineno, exc.msg))
        elif kind == "requirements": requirements[rel] = text
    for digest, paths in hashes.items():
        useful = [p for p in paths if not p.endswith(("__init__.py", ".gitkeep"))]
        if len(useful) > 1: findings.append(Finding("low", "FILE-DUPLICATE", useful[0], 1, f"identical content shared by {useful}; sha256={digest}"))
    ...
    for rel, text in requirements.items():
        for index, raw in enumerate(text.splitlines(), 1):
            ...
            if not match: findings.append(Finding("medium", "REQ-UNPINNED", rel, index, f"dependency is not constrained: {value}")); continue
