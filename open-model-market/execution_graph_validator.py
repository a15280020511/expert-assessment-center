...
def _walk_keys(value: Any, path: str = "request_config") -> Iterable[tuple[str, str]]:
...
def _stage_index(graph: ExecutionGraph) -> tuple[dict[str, int], list[ValidationIssue]]:
...
def derive_execution_stages(graph: ExecutionGraph) -> tuple[tuple[str, ...], ...]:
    """Return deterministic topological generations for an already valid DAG."""
    dag = nx.DiGraph()
    ...
    return tuple(tuple(sorted(generation)) for generation in nx.topological_generations(dag))
...
def validate_execution_graph(
    ...
    *,
    ...
    if len(graph.nodes) > limits.max_nodes:
        issues.append(ValidationIssue("node_limit", f"Node count exceeds {limits.max_nodes}.", "nodes"))
    if len(graph.edges) > limits.max_edges:
        issues.append(ValidationIssue("edge_limit", f"Edge count exceeds {limits.max_edges}.", "edges"))
    ...
    for index, node in enumerate(graph.nodes):
        ...
        if node.estimated_quality < 0 or node.estimated_quality > 1:
            issues.append(ValidationIssue("node_quality_range", "Estimated quality must be in [0, 1].", path))
        if node.quality_uncertainty < 0 or node.quality_uncertainty > 1:
            issues.append(ValidationIssue("uncertainty_range", "Quality uncertainty must be in [0, 1].", path))
        if node.failure_probability < 0 or node.failure_probability > 1:
            issues.append(ValidationIssue("failure_range", "Failure probability must be in [0, 1].", path))
