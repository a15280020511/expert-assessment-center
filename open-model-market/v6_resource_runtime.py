"""V6 prompt-led resource governance without local token or cost stop gates."""
...
def _without_local_token_caps(
    ...
    if isinstance(messages, list) and messages and isinstance(messages[0], Mapping):
        ...
        first["content"] = str(first.get("content") or "") + V6_RESOURCE_INSTRUCTION
        updated[0] = first
        softened["messages"] = updated
    return softened


class V6ResourcePromptPolicy(ConstitutionalPromptPolicy):
    def build_payload(
        self,
        node: SelectedNode,
        original_task: str,
        ...
        return _without_local_token_caps(
            super().build_payload(node, original_task, upstream),
            node,
        )


class V6ResourceBudgetController(BudgetController):
    """Semantic alias for the constitutionally soft base controller."""


class V6ResourceExecutionEngine(ConstitutionalExecutionEngine):
    def _preflight(
        self,
        ...
        value.update(
            {
                ...
                "resource_governance_mode": "v6-prompt-led-soft-governance",
            }
        )
        return value
    ...
    def _execute_graph_soft(
        ...
        *,
        ...
        budget = V6ResourceBudgetController(self.config, graph)
        ...
        result["resource_governance"] = {
            "mode": "v6-prompt-led-soft-governance",
...
def _v6_recovery_policy() -> RecoveryPolicy:
...
def harden_runtime(runtime: ProductionRuntime) -> ProductionRuntime:
    runtime.recovery_policy = _v6_recovery_policy()
    runtime.prompt_policy = V6ResourcePromptPolicy()
    runtime.execution_engine = V6ResourceExecutionEngine(
        runtime.config,
    ...
    *,
    ...
    return harden_runtime(
        ProductionRuntime(
            ...
            recovery_policy=_v6_recovery_policy(),
        )
    )
...
__all__ = [
    "V6_RESOURCE_INSTRUCTION",
    "V6ResourceBudgetController",
    "V6ResourceExecutionEngine",
    "V6ResourcePromptPolicy",
    "build_runtime",
    "harden_runtime",
]
