...
def diversity_aware_preflight(
    ...
    """Apply runtime substitutions without invalidating hard invariants."""
    recovery = (
    ...
    def company_safe(node_id: str, alternative: Any) -> bool:
        """Do not introduce a model company used by any peer node."""
        alternative_company = candidate_company(alternative)
        return all(
            peer.node_id == node_id
            or candidate_company(peer) != alternative_company
            for peer in nodes.values()
        )
    ...
    for selected in graph.nodes:
        ...
        if (
            active.failure_probability
            <= limits.max_node_failure_probability
        ):
            continue
        rows = (
            recovery.get(selected.node_id, [])
            if isinstance(recovery, Mapping)
            else []
        )
        alternatives = [
            runtime._candidate(row, selected) for row in rows
        ]
        alternatives = [
            item
            for item in alternatives
            if item.failure_probability < active.failure_probability
            and item.failure_probability
            <= limits.max_node_failure_probability
        ]
        alternatives.sort(
            key=lambda item: (
        ...
        safe = next(
            (
                ...
                and company_safe(selected.node_id, item)
            ),
            None,
        )
        if safe is not None:
            ...
            substitutions.append(
                {
                    ...
                    "reason": (
                        "failure-probability-above-production-threshold"
                    ),
                    "from": previous.provider_endpoint,
                    "to": safe.provider_endpoint,
                    "from_model_company": candidate_company(previous),
                    "to_model_company": candidate_company(safe),
                }
            )
            continue
        for alternative in alternatives:
            if not company_safe(selected.node_id, alternative):
                reason = (
                    "reliability-replacement-would-break-model-company-"
                    "diversity"
                )
            elif not independence_safe(selected.node_id, alternative):
                reason = (
                    "reliability-replacement-would-break-independent-"
                    "model-diversity"
                )
            elif not budget_safe(selected.node_id, alternative):
                reason = (
                    "reliability-replacement-would-exceed-raw-budget"
                )
            else:
            ...
            rejected_substitutions.append(
                {
                    ...
                    "candidate_model_company": candidate_company(
                        alternative
                    ),
                }
            )
        if (
            selected.node_id in graph.final_nodes
            or "synthesis" not in selected.functions
        ):
            blockers.append(
                f"required-node-risk-above-threshold:{selected.node_id}"
            )
        else:
            warnings.append(
                f"optional-node-risk-above-threshold:{selected.node_id}"
            )
            ...
            for node_id, node in nodes.items():
                ...
                for row in rows:
                    ...
                    if (
                        ...
                        continue
                    if not company_safe(node_id, alternative):
                        unsafe.append(
                            (
                                node_id,
                                alternative,
                                (
                                    "provider-rebalance-would-break-model-"
                                    "company-diversity"
                                ),
                            )
                        )
                    ...
                    if not independence_safe(node_id, alternative):
                        unsafe.append(
                            (
                                ...
                                alternative,
                                (
                                    "provider-rebalance-would-break-"
                    ...
                    if not budget_safe(node_id, alternative):
                        unsafe.append(
                            (
                                ...
                                alternative,
                                (
                                    "provider-rebalance-would-exceed-raw-"
                                    "budget"
                                ),
                            )
                        )
            ...
            substitutions.append(
                {
                    ...
                    "from_model_company": candidate_company(previous),
                    "to_model_company": candidate_company(alternative),
                }
            )
    ...
    for node in nodes.values():
        company = candidate_company(node)
        company_counts[company] = company_counts.get(company, 0) + 1
    max_share = max(provider_counts.values(), default=0) / max(
        1,
        len(nodes),
    )
    strict = bool(
    ...
    if (
        len(nodes) >= 3
        and max_share > limits.max_provider_share + 1e-12
    ):
        if strict:
            blockers.append(
    ...
    risk_cost = rebuilt.estimated_total_cost * max(
        1.0,
        limits.cost_risk_multiplier,
    )
    if (
        limits.max_budget_usd is not None
        and risk_cost > limits.max_budget_usd + 1e-12
    ):
        blockers.append("preflight-risk-adjusted-cost-above-hard-budget")

    structural_issues = [
        issue
        for issue in validate_execution_graph(rebuilt, limits)
    ...
    report = {
        ...
        "model_company_counts": company_counts,
        "model_company_reuse_allowed": False,
        ...
        "blockers": blockers,
        "policy": (
            "R8 budget-safe reliability/provider preflight with hard "
            "model-company diversity and post-substitution validation"
        ),
    }
