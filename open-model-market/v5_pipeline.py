...
from v5_model_company_policy import model_company
...
def build_parser() -> argparse.ArgumentParser:
    ...
    parser.add_argument("--maximum-candidates-per-work", type=int, default=24)
...
def _annotate_market(
    ...
    *,
    ...
    companies = [model_company(model.id, getattr(model, "author", None)) for model in ranked]
    ...
    compiled_market["candidate_pool_policy"] = "multi-channel-company-diverse-before-optimizer"
    ...
    compiled_market["ranked_models"] = [
        {
            "rank": index,
            "model": model.id,
            "model_company": model_company(model.id, getattr(model, "author", None)),
