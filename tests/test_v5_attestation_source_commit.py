...
class V5AttestationSourceCommitTests(unittest.TestCase):
    def test_checked_out_commit_sha_reads_exact_git_head(self) -> None:
        ...
        with mock.patch.object(attestation.subprocess, "run", return_value=completed) as run:
    ...
    def test_checked_out_commit_sha_rejects_non_full_sha(self) -> None:
        ...
        with mock.patch.object(attestation.subprocess, "run", return_value=completed):
    ...
    def test_main_attests_checkout_not_issue_event_context(self) -> None:
        with tempfile.TemporaryDirectory(prefix="v5-attestation-source-") as directory:
            ...
            self.assertEqual(EVENT_CONTEXT_SHA, record["event_context_commit_sha"])
            self.assertFalse(record["event_context_commit_matched_checkout"])
