...
def node(
    ...
    *,
    ...
    model = model or f"company-{name}/model-{name}"
    return SelectedNode(
        node_id=name,
    ...
    value.update(
        {
            ...
            "provider_endpoint": f"{model}@{provider}",
            "estimated_cost": (
                selected.estimated_cost if cost is None else cost
            ),
            "failure_probability": (
...
class TestProviderPolicy(unittest.TestCase):
    def test_concentration_without_alternative_warns_but_keeps_availability(self):
        ...
        self.assertEqual(
            report["post_substitution_validation"]["status"],
            "PASS",
        )
        self.assertFalse(
        ...
        self.assertEqual(len(report["model_company_counts"]), 3)

    def test_recovery_pool_rebalances_provider_before_execution(self):
        nodes = tuple(node(name, "p1") for name in ("a", "b", "c"))
        alternative = candidate(
            nodes[0],
            model="replacement-a/model-a2",
            provider="p2",
        )
        adjusted, report = policy.diversity_aware_preflight(
        ...
        self.assertEqual(
            adjusted.nodes[0].provider_endpoint,
            "replacement-a/model-a2@p2",
        )
        self.assertEqual(
            report["substitutions"][0]["to_model_company"],
            "replacement-a",
        )

    def test_expensive_provider_rebalance_is_rejected(self):
        nodes = tuple(
            node(name, "p1", cost=0.02) for name in ("a", "b", "c")
        )
        expensive = candidate(
            nodes[0],
            model="replacement-a/model-a2",
    ...
    def test_provider_rebalance_cannot_duplicate_model_or_company(self):
        nodes = (
            node(
                "independent-a",
                "p1",
        ...
        self.assertEqual(report["status"], "pass")
        self.assertEqual(adjusted.nodes[1].model, "company-b/model-b")
        self.assertEqual(
            report["post_substitution_validation"]["status"],
            "PASS",
        )
        self.assertTrue(
            any(
                row["reason"]
                == "provider-rebalance-would-break-model-company-diversity"
                for row in report["rejected_substitutions"]
            )
        )
        self.assertFalse(
            any(
                issue.code
                in {"independent_same_model", "model_company_reuse"}
                for issue in validate_execution_graph(
                    adjusted,
                    GraphLimits(max_provider_share=0.60),
                )
            )
    ...
    def test_different_model_same_company_is_rejected(self):
        nodes = tuple(node(name, "p1") for name in ("a", "b", "c"))
        unsafe = candidate(
            nodes[1],
            model="company-a/another-model",
            provider="p2",
            cost=0.005,
            failure=0.01,
        )
    ...
    def test_reliability_replacement_preserves_company_diversity(self):
        nodes = (
            node(
                "a",
                "p1",
        ...
        self.assertIn(
            "required-node-risk-above-threshold:b",
            report["blockers"],
        )
        self.assertTrue(
            any(
                row["reason"]
                == "reliability-replacement-would-break-model-company-diversity"
                for row in report["rejected_substitutions"]
            )
        )

    def test_strict_provider_diversity_fails_closed_without_alternative(self):
        nodes = tuple(
            node(name, "p1", cost=0.02) for name in ("a", "b", "c")
        )
        _, report = policy.diversity_aware_preflight(
            graph(
                nodes,
                metadata={"provider_diversity_required": True},
            ),
            GraphLimits(
                max_provider_share=0.60,
                max_budget_usd=0.10,
