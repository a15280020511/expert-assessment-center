...
class TaskDerivedParameterGraphTests(unittest.TestCase):
    def test_parameter_instances_change_with_current_task_graph(self) -> None:
        ...
        self.assertNotIn("dependency_density", simple_ids)
        self.assertNotIn("parallelism_ratio", simple_ids)
        self.assertIn("dependency_density", branched_ids)
        self.assertIn("parallelism_ratio", branched_ids)
        ...
        self.assertEqual(0, audit["unconsumed_parameter_count"])
        self.assertTrue(audit["every_parameter_has_active_consumer"])
    ...
    def test_role_dependencies_are_derived_from_work_dag_not_three_role_grammar(self) -> None:
        ...
        self.assertFalse(
            {row["role_kind"] for row in roles}.issubset(
    ...
    def test_orchestrator_preserves_explicit_arbitrary_role_dag(self) -> None:
        ...
        proposal, audit = build_governed_proposal(
            ticket=ticket, catalog={}, task_envelope={}
        )
        self.assertEqual(
...
class ProductionEntryRegressionTests(unittest.TestCase):
    ...
    def test_production_soft_entry_initializes_standby_and_uses_graph_call_capacity(self) -> None:
        ...
        graph = ExecutionGraph(
            ...
            metadata={
                ...
                "standby_inventory": [
                    {
                        "model": "vendor/c",
                        "provider_endpoint": "vendor/c@openrouter-auto",
                    },
                    {
                        "model": "vendor/d",
                        "provider_endpoint": "vendor/d@openrouter-auto",
                    },
                ],
            },
        )
        ...
        engine = Probe()
        result = engine._execute_graph_soft(
            graph,
            object(),
            "task",
            limits=GraphLimits(),
        ...
        self.assertFalse(
            result["resource_governance"]["requested_call_ceiling_can_stop_execution"]
        )
