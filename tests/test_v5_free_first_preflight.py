...
def valid_shadow_attestation():
    return {
        "status": "PASS",
        "target_sha": TARGET_SHA,
        "shadow_mode": "single-endpoint-zdr-compatibility",
...
def valid_receipt():
    return {
        ...
        "free_canary": {
            "status": "PASS",
            "requested_model": "inclusionai/ling-3.0-flash:free",
            "model_requests": 1,
        ...
        "shadow_attestation": valid_shadow_attestation(),
...
class FreeFirstPreflightTests(unittest.TestCase):
    def test_complete_free_first_receipt_allows_only_paid_acceptance(self):
        ...
        self.assertTrue(verdict["independently_revalidated_free_shadow_passed"])
    ...
    def test_free_canary_fallback_blocks_authorization(self):
        receipt = valid_receipt()
        receipt["free_canary"]["provider_fallback_allowed"] = True
        verdict = preflight.evaluate_free_first_preflight(receipt)
        self.assertEqual(verdict["status"], "FAIL")
        self.assertIn("free-canary-fallback-allowed", verdict["reasons"])
    ...
    def test_shadow_attestation_is_mandatory(self):
        receipt = valid_receipt()
        receipt["shadow_attestation"] = None
        verdict = preflight.evaluate_free_first_preflight(receipt)
        self.assertEqual(verdict["status"], "FAIL")
        self.assertIn("shadow-attestation-required", verdict["reasons"])
        self.assertFalse(verdict["independently_revalidated_free_shadow_passed"])

    def test_shadow_target_must_match_paid_target(self):
        receipt = valid_receipt()
        receipt["shadow_attestation"]["target_sha"] = "b" * 40
        verdict = preflight.evaluate_free_first_preflight(receipt)
        self.assertEqual(verdict["status"], "FAIL")
        self.assertIn("shadow-target-sha-mismatch", verdict["reasons"])

    def test_shadow_requires_four_free_calls(self):
        receipt = valid_receipt()
        receipt["shadow_attestation"]["successful_free_model_calls"] = 3
        verdict = preflight.evaluate_free_first_preflight(receipt)
        self.assertEqual(verdict["status"], "FAIL")
        self.assertIn("shadow-success-count-invalid", verdict["reasons"])

    def test_shadow_requires_independent_primitive_recomputation(self):
        receipt = valid_receipt()
        receipt["shadow_attestation"]["independent_revalidation"][
            "recomputed_from_primitive_evidence"
        ] = False
        verdict = preflight.evaluate_free_first_preflight(receipt)
        self.assertEqual(verdict["status"], "FAIL")
        self.assertIn(
            "shadow-not-recomputed-from-primitive-evidence",
            verdict["reasons"],
        )

    def test_shadow_requires_expert_company_uniqueness(self):
        receipt = valid_receipt()
        receipt["shadow_attestation"]["expert_company_uniqueness_required"] = False
        verdict = preflight.evaluate_free_first_preflight(receipt)
        self.assertEqual(verdict["status"], "FAIL")
        self.assertIn(
            "shadow-expert-company-uniqueness-not-required",
            verdict["reasons"],
        )

    def test_shadow_cannot_claim_formal_model_identity(self):
        receipt = valid_receipt()
        receipt["shadow_attestation"]["formal_model_identity_qualified"] = True
        verdict = preflight.evaluate_free_first_preflight(receipt)
        self.assertEqual(verdict["status"], "FAIL")
        self.assertIn(
