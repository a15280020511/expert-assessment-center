...
from publish_report import strict_publication_gate, write_comments  # noqa: E402
from v5_gpt_expert_selector_policy import (  # noqa: E402
...
class DegradedPolicyTests(unittest.TestCase):
    def test_default_and_explicit_denial(self) -> None:
        self.assertTrue(
            compile_task_constraints("分析并建议").allow_degraded_success
        )
        self.assertFalse(
            compile_task_constraints("不得降级交付").allow_degraded_success
        )

    def test_single_node_three_recoveries(self) -> None:
        parsed = parse_proposal(json.dumps(proposal(3)))
        self.assertEqual(3, len(parsed["nodes"][0]["recovery"]))
        with self.assertRaisesRegex(RuntimeError, "node recovery"):
            parse_proposal(
                json.dumps(
                    proposal(MAXIMUM_RECOVERY_CANDIDATES_PER_NODE + 1)
                )
            )
    ...
    def test_failed_noncritical_node_is_disclosed(self) -> None:
        ...
        self.assertEqual(
            "DEGRADED",
            normalized["quality_integrity"]["status"],
        )
        self.assertEqual(
            ["N2"],
            normalized["quality_integrity"]["failed_node_ids"],
        )

    def test_normative_fact_label_is_repaired(self) -> None:
        normalized, audit = relabel_task_derived_fact_lines(
            "用户优先稳定、低投入；不得立即辞职。",
            "- 事实：用户优先稳定、低投入，因此不得立即辞职。\n",
        )
    ...
    def test_degraded_publication_gate(self) -> None:
        with tempfile.TemporaryDirectory() as directory:
            ...
            report.write_text(
                "usable audited degraded report",
                encoding="utf-8",
            )
